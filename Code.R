---
  title: "Report for Final Presentation"
author: 'Team 11: Brian Lin, Lily Damron, Aida Ylanan, Liam Carrigan, Ignat Kulinka'
...
listings <- read_csv("~/Desktop/UCLA/Fall 2017/STATS 140SL/Homework 8/Dataset/listings.csv")
...
plot <- ggplot(listingsWithRules, aes(x = numRules)) +
  geom_bar(aes(fill = rulesCleaned)) +
  theme_bw() + 
...
a <- listingsUpdatedDF %>%
  ...
  kable(., format = "latex",
        booktabs = TRUE,
        digits = 2,
        col.names = c("Region", "Mean Strictness", "Mean Price",
                      "No Smoking", "No Party","No Noise", "No Drugs", "No Shoes")) %>% 
  landscape() %>% 
  kable_styling(full_width = F, font_size = 10) %>% 
  kable_as_image(filename = "~/Desktop/UCLA/Fall 2017/STATS 140SL/Final Report/table", file_format = "png") %>% table2tex()


#Unfortunate there’s only 1 observation in Angeles Forest after all NA removed.
```

```{r exploratoryMaps, include=FALSE}
# Exploratory maps --------------------------------------------------------
dataJson <- geojson_read(
  "~/Desktop/UCLA/Fall 2017/STATS 140SL/Homework 8/Dataset/neighbourhoods.geojson", what = "sp")
...
#before
dataJsonOne <- geojson_read(
  "~/Desktop/UCLA/Fall 2017/STATS 140SL/Homework 8/Dataset/neighbourhoods.geojson", what = "sp")
#after
dataJsonTwo <- geojson_read(
  "~/Desktop/UCLA/Fall 2017/STATS 140SL/Homework 8/Dataset/la-county-regions-current.geojson", what = "sp")

mapImage <- ggmap(get_googlemap(c(lon = -118.2437, lat = 34.0522), scale = 2, 
                                zoom = 9, maptype = "roadmap"), extent = "panel")
mapBefore <- mapImage +
  geom_polygon(data = dataDF_ONE, aes(long, lat, group = group),
...
#save the maps 
graph2png(mapBefore, aspectr=sqrt(2))
graph2png(mapAfter, aspectr=sqrt(2))

dataDF_ONE <- fortify(dataJsonOne)          
...
graph2png(mapOne, aspectr=sqrt(2))
...
graph2png(mapTwo, aspectr=sqrt(2))
#to save the graph
#ggsave("~/Desktop/UCLA/Fall 2017/STATS 140SL/Homework 8/mapImageSeven.png", bg = "transparent")
...
#save mapThree
graph2png(mapThree, aspectr=sqrt(2))


#ggsave("~/Desktop/UCLA/Fall 2017/STATS 140SL/Homework 8/mapImageSix.png", bg = "transparent")
